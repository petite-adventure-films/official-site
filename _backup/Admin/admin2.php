...
<?php
	...
	if(isset($_POST['execute']) || isset($_GET['newsid']))
	{
		...
		if(isset($_GET['func']))
		{
			...
			{
				$query = 'SELECT * from news WHERE idnews = ' . $_GET['newsid'];
			...
			{
				$query = 'UPDATE news SET news_status = 0 WHERE idnews = ' . $_GET['newsid'];
				$result = mysql_query($query);
				if(!$result)
				{
					print "Edit Query failed" . mysql_error();
				...
				header("Location:admin2.php?queryyear=" . $queryYear);
				exit;
			}
			if(strcmp($_GET['func'],'undelete') === 0)
			{
				$query = 'UPDATE news SET news_status = 1 WHERE idnews = ' . $_GET['newsid'];
				$result = mysql_query($query);
				if(!$result)
				{
					print "Edit Query failed" . mysql_error();
...
<div id="wrapper">
	...
	<div id="menu">
		<ul>
			...
			<li class="current_page_item"><span><a href="admin2.php">Add News</a></span></li>
		...
		<div id="page-bgtop">
			<div id="page-bgbtm">
				<div id="content">
					...
					?>
					<form accept-charset="UTF-8,ISO-8859-1" class="standardform" action=<?php echo '"admin2.php?queryyear=' . $queryYear . '"';?> method="post">
					<table>
						<tr>
							<td>Language</td>
							<td>
				...
				<?php
					...
					$userConnection = ConncetDB();
					$query = 'SELECT * FROM `news` WHERE EXTRACT(YEAR FROM `news_posted_date`) = ' . $queryYear . ' ORDER BY `news_posted_date` DESC';
					$result = mysql_query($query);
					if (!$result)
					{
					...
					while($row = mysql_fetch_assoc($result))
					{
					    ...
					    {
						echo 'news deleted: <a href="admin2.php?queryyear=' . $queryYear . '&func=undelete&newsid=' . $row['idnews'] . '">undelete</a>'; 	
					    }
					    else
					    {
						echo '<a href="admin2.php?queryyear=' . $queryYear . '&func=edit&newsid=' . $row['idnews'] . '">edit</a>' . ' - ';
						echo '<a href="admin2.php?queryyear=' . $queryYear . '&func=delete&newsid=' . $row['idnews'] . '">delete</a>'; 
					    }
					    echo '<hr/>';
					}
					mysql_close($userConnection);
					...
					{
						echo 'To cancel edit click <a href="admin2.php?queryyear=' . $queryYear . '">*here*</a>'; 
						echo '<hr/>';
					}
				?>
					<form accept-charset="UTF-8,ISO-8859-1" class="standardform" action="admin2.php" method="post">
