<div class="m-4 p-4 border border-red-600 text-red-600">
  pdfプレビューは機能しません。
</div>
<div class="flex">
    ...
    ?>
        ...
        <div class="mt-8">
          ...
          <div class="flex flex-wrap gap-x-2 mt-2 text-xs"><!--[-->
            <span class="before:content-['#'] before:pr-[1px]"><?= get_post_meta(get_the_ID(), 'media_info_01', TRUE); ?></span>
            <span class="before:content-['#'] before:pr-[1px]"><?= get_post_meta(get_the_ID(), 'media_info_02', TRUE); ?></span>
            ...
            <article class="relative mt-8 border border-black py-8 px-4 before:content-['記事本文'] before:text-xs before:px-2 before:py-1 before:bg-gray-100 before:absolute before:right-0 before:top-0">
              <h3 class="mb-8">
                <span class="font-bold">
                  <?= get_post_meta(get_the_ID(), "media_info_06", TRUE); ?>
                </span>
                <? if (get_post_meta(get_the_ID(), "media_info_07", TRUE)) { ?>
                  <span class="block text-xs">
                    <?= get_post_meta(get_the_ID(), "media_info_07", TRUE); ?>
                  </span>
                <? } ?>
              </h3>
              <div>
                <?= get_post_meta(get_the_ID(), "media_info_08", TRUE); ?>
              </div>
            </article>
          <? } ?>
          <div class="mt-8">
