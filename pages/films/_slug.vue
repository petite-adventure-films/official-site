<template>
	<div>
		...
		<div v-if="post.fields.awards">
			...
			:key="'award' + arr.key">
				{{arr.key}}<br>
				<span v-for = "val in arr.data" :key="'award' + arr.key + val">{{val}}</span>
			</div>
		</div>
		...
		<div
		v-for = "arr in post.fields.details"
		:key = "'details' + arr.key">
			{{arr.key}}: <span v-for = "val in arr.data" :key="'details' + arr.key + val">{{val}}</span>
		</div>
		...
		<div v-if="post.fields.recommends">
			...
			<h4>推薦のことば</h4>
			<div
			v-for = "arr in post.fields.recommends"
			:key  = "'recommends' + arr.key">
		...
		<h4>制作クレジット</h4>
		<div
		v-for = "arr in post.fields.credit"
		:key = "'credit' + arr.key">
			<h5>{{arr.key}}</h5>
			<div
			v-for = "(data, index) in arr.data"
			:key = "'credit' + arr.key + index">
				<!-- さらにオブジェクト -->
				<div v-if="typeof data == 'object'">
					{{data.key}}
					<div
					v-for = "(val2, key2) in data.data"
					:key = "'credit' + arr.key + val2">
		...
		<div v-if="post.fields.domesticFestivals || post.fields.internationalFestivals">
			...
			<div v-if="post.fields.domesticFestivals">
				...
				:key = "'domestciFestivals' + arr.key">
					<span>{{arr.key}}</span>
					<span
					v-for = "(val, key) in arr.data"
					:key = "'domestciFestivals' + arr.key + key">
						{{val}}
					</span>
				</div>
			</div>
			<div v-if="post.fields.internationalFestivals">
				...
				:key = "'internationalFestivals' + arr.key">
					<span>{{arr.key}}</span>
					<span
					v-for = "(val, key) in arr.data"
					:key = "'internationalFestivals' + arr.key + key">
						{{val}}
					</span>
				</div>
			</div>
		...
		<h4>関連情報</h4>
		<div
		v-for = "arr in post.fields.relatedInfo"
		:key = "'relatedInfo' + arr.key">
...
export default {
	...
	, async asyncData({ payload, store, params, error }) {
		const post = await store.state.film.find(post => post.fields.slug === params.slug);
		if (post) {
			return { post }
		} else {
