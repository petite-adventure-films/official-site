<form v-if="step == 1">
    ...
    <div>
        <input type="text" v-model="user.zipcode" placeholder="郵便番号 例）1000005" @input="searchAddress()"><br>
        <span v-if="errors.zipcode !== false">{{errors.zipcode}}</span>
    </div>
    <div>
        <input type="text" v-model="user.prefecture" placeholder="都道府県 例）東京都"><br>
        <span v-if="errors.prefecture !== false">{{errors.prefecture}}</span>
    </div>
    <div>
        <input type="text" v-model="user.city" placeholder="市区町村 例）千代田区丸の内"><br>
        <span v-if="errors.city !== false">{{errors.city}}</span>
    </div>
    <div>
        <input type="text" v-model="user.address1" placeholder="番地 例）1-1-1"><br>
        <span v-if="errors.address1 !== false">{{errors.address1}}</span>
    </div>
    <div>
    ...
    <div>
        ...
        <span v-if="errors.tel !== false">{{errors.tel}}</span>
    </div>
    <div>
        <input type="email" v-model="user.email" placeholder="メールアドレス"><br>
        <span v-if="errors.email !== false">{{errors.email}}</span>
    </div>
    <div>
        <input type="email" v-model="user.emailConfirm" placeholder="メールアドレス確認"><br>
        <span v-if="errors.emailConfirm !== false">{{errors.emailConfirm}}</span>
    </div>

    <div>
        <input type="checkbox" v-model="user.receipt" id="receipt"><label for="receipt">領収書必要</label><br>
        <input v-if="user.receipt == true" type="text" v-model="user.receiptName" placeholder="お宛名"><br>
        <input v-if="user.receipt == true" type="text" v-model="user.receiptDescription" placeholder="但し書き">
    </div>
    ...
    <div>
        ...
        <div class="contents_law">
            ...
            echo $_contents_law;
            ?>
            <input type="checkbox" v-model="user.agree" id="agree"><label for="agree">上記個人情報の取扱について同意しました</label><br>
            <span v-if="errors.agree !== false">{{errors.agree}}</span>
        </div>
    </div>
